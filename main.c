...
bool list(limb * limbo){
    ...
    printLimb(limbo);   //limbo di senza padre
    return status >= 0;
}
...
bool tie(int idChild, int idParent, limb * limbo){
    ...
    else if (idParent == 0){    //Linking to Centralina
        printf("Linking to centralina, device n: %d\n", idChild);
        int childType = tmp->type;
        spawn(idParent, childType, idChild);    //cosa sarebbe il primo argomento? Possiamo toglierlo?
    }


    // controlla che parent sia controller
        //manda a idParent una richiesta per chiedergli il suo type, aspetta la risposta e mettila in type col solito modo ciclico sui figli
    // parte in broadcast una richiesta
        //cicla sulla lista delle pipe dei figli un messaggio di spawn(idChild, type) con destinatario idParent
        ...
        if (fgets(buffer, MAXLEN, stdin) != NULL) {
            ...
            if (strcmp(tokens[0], "list\n")==0) {
                ...
                if (tokens[1] != NULL && ((strcmp(tokens[2], "to") == 0) && tokens[3] != NULL)){
                    printf("DIOMA");
                    int idChild = atoi(tokens[1]);
                    printf("id child to be linked is %d\n", idChild);
                    status = tie(idChild, idParent, limbo);
